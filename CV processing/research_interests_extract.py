# This script takes a directory of CV files, extracts the name on the CV and their research interests, stores it them in a pandas datatable, then saves that data as a csv. The idea is that we then take the csv and directly upload the data to the mysql database.
import re
...
departments = ['africana studies', 'asian studies', 'biological sciences', 'biology', 'chemistry', 'computer science', 'english', 'fire administration', 
               ...
               'theatre and film', 'world languages & cultures', 'american culture studies', 'ethnic studies', 'popular culture', 
               "women's, gender, & sexuality studies"]

departments_varients = ['mathematics & statistics', 'mathematics', 'statistics', 'physics & astronomy', 'physics', 'astronomy', 
                        'theatre & film', 'theatre', 'film', 'world languages and cultures', "women's, gender, and sexuality studies"]

# mapping used to unify department names
varient_mapping = {'mathematics & statistics': 'mathematics and statistics', 'mathematics': 'mathematics and statistics', 
                   'statistics': 'mathematics and statistics', 'physics & astronomy': 'physics and astronomy', 'physics': 'physics and astronomy', 
                   'astronomy': 'physics and astronomy', 'theatre & film': 'theatre and film', 'theatre': 'theatre and film', 'film': 'theatre and film',
                   'world languages & cultures': 'world languages and cultures', "women's, gender, and sexuality studies": 
                    "women's, gender, & sexuality studies"}
...
def get_department(raw_text):
    ...
    try:
        ...
        department = department.replace('professor','').replace('of','').replace(',','').strip() # there might be 2 spaces between 'computer  science'
    ...
    if max(frequency.values()) >= max(frequency_varients.values()):
        ...
        return varient_mapping[max(frequency_varients, key=frequency_varients.get)].title() # return the unified department name

cv_directory = r'G:\Python File Saves\Capstone Project\cv_data' # root folder
files = glob.glob(cv_directory+'/*.pdf')
df = pd.DataFrame([])
for file in files:
    print(file)
    ...
    name = name.replace('curriculum vitae','') # if curriculum vitae is in the first line with their name, just remove it
    
    department = get_department(raw_text)
    interests = get_interests(raw_text,False) # list of interests
    if interests == '': pass
    li = []
    for i in range(len(interests)):
        li.append([name,department,interests[i]])
    df = df.append(li)
df.rename(columns={0:'name',1:'department',2:'interest'},inplace=True)
#df.to_csv('_'.join(clean_text(name).split(' '))+'_cv.csv',index=False) # saves a csv file with the name of the person
